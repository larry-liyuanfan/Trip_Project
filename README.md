...
逐周报告、周日志、会议材料和历史计划不进入最终工作树；需要时可通过 Git 历史查阅。

## 分支职责

| 分支 | 用途 | 晋级规则 |
| --- | --- | --- |
| `dev` | 日常开发与集成 | 功能分支合入后执行完整验证 |
| `stg` | 稳定候选 | 仅接收已验证的 `dev` 提交 |
| `main` | 最终提交 | 仅接收确认交付的 `stg` 提交 |
...
GPU 训练和推理依赖独立维护在 `requirements-llm.txt`、`requirements-training.txt` 和
`requirements-training-spartan-cu128.txt`。不要在原生 Windows Python 中默认安装 vLLM。
...
当前正式交付记录为 948/948 单元测试通过。修改代码后以当前工作树的实际运行结果为准。
...
数据层和评测契约见 `docs/yelp_dataset.md`、`docs/evaluation_data_contracts.md`、
`docs/evaluation_framework.md` 与 `docs/evaluation_metrics.md`。
