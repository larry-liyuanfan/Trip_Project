...
暂固定为 `qwen3_vl_system_week8_v10.json`。其状态仍是 candidate_evaluation；真实业务、
...
逐错误语义不退已通过，原固定 60 图完整质量对比也通过。现选定 v11 待验，
...
后续 `product_observation_subject_review_v1.json` 是尚未验收的development实验：对部分
场景独立复查主体功能，保持原风格/设施/价位，矛盾仍失败；不作为正式默认配置。
完整898条unittest通过，真实商品效果以 `contract_ablation_v15` 的实测结果为准。
