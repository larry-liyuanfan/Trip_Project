...
As of 2026-07-24, the approved v2 supplements, full baseline, full standardized run, scoring, and same-set comparison are complete and traceable. Week 3 remains `PARTIAL` only because the required minimal baseline produced natural-language output whose semantic task fields have no approved deterministic coding rule; those metrics remain `PENDING` rather than numeric zero.
