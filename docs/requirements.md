...
### 2026-08-26 用户解除范围限制与全自动执行授权

- 用户最新指令解除上述关于执行顺序、实验次数、训练方法、模型换代、复杂重排、服务架构
  调整和仅限既有四项实现深度的限制。允许围绕当前已发现问题自主采用数据重建、两阶段
  商品架构、专项 SFT、自动偏好优化、强模型教师/候选比较、确定性对话路由、缓存/量化、
  hybrid/Milvus 检索和发布监控；只记录实际运行和可复现结果。
- 从本条生效后不再安排或等待任何人工标注、人工复核、人工偏好选择、人工验收或人工
  抽样评价。本轮新增标签、证据、偏好对和相关性目标的人工参与数固定为 `0`，统一保留
  `silver`、`programmatic_silver` 或 `model_generated_silver` 身份，不得写成 human gold、
  human accepted 或人工结论。历史已有 human 身份只按原记录引用，不由本轮改写。
- 无法在无人工条件下获得可靠真值的字段必须改为自动证据边界、弱监督或 `unknown` 任务，
  并单独报告支持数与局限；不得用模型自评冒充人工正确性。
- 继续保留数据身份隔离、不可覆盖历史冻结资产、真实结果记录、密钥保护和非破坏性操作。
...
### 2026-08-27 c01b732 审查缺陷修复

- Week 8 保持 PARTIAL，不晋级当前候选。按用户审查修复九项确定性缺陷：版本化标签的
  结构化属性/完整词/否定处理、视觉选优资格、对话实际分派、行程业务验收、完整金额解析、
  轮次图片绑定、真实查询检索闭环、场景输入 422、CLI/Compose/服务统一 release 解析。
- 商家 metadata 与可见事实分离；旧标签只按明确历史协议复现，不覆盖冻结数据。metadata
  匹配只作诊断，缺少价位支持单列 N/A，不阻止有可靠参考的其他字段优化。
- 保留确定性状态更新，但确认语不得代表任务完成；执行失败返回明确未完成。行程检查
  天数、日序、明确约束和占位内容，使用既有一次纠错，技术通过与业务验收分别记录。
- 不扩训练规模、不新增人工工作、不消费已使用 final；增加审查反例和端到端回归证据。

### 2026-08-28 持续复审至可晋级候选

- 用户要求持续 review、修复和验证，直到形成完整通过的可晋级候选；上一条 PARTIAL
  是已审查候选的真实状态，不限制后续修复。先纠正 Prompt、运行契约和业务验收的冲突，
  再用 development 比较模型方案，不以扩大训练掩盖确定性缺陷。
- 允许在既有授权内使用底座消融和独立图像教师，但自动参考仍为 silver，不能将教师
  一致性或 metadata 匹配宣称为人工视觉正确率。没有可靠支持的字段单列 N/A。
...
- 用户要求自主寻找并持续验证优化路径，不能以一批未胜出实验代替优化目标完成。
  以 v9 为当前比较对象，保持所有有支持字段不回退，取得商品综合分提升或质量不退的
  可复现实质性能收益后，再完成适用的独立候选验收。没有实际提升不得标记目标完成。
- 当前先验证字段级视觉复查：原商品判断保留，独立检查风格的漏识别和无依据扩展；
  新协议不得使用教师标签作为输入、悄悄删除原始错误或更改其他字段来隐藏回退。
- 推理参数优化须核验真实生效路径与原始输出，不假设缩短参数或输出天然提速。
  零人工工作、已消费 final 不调参、冻结 v9 不覆盖和 feature-only 边界继续有效。
- 持续复审若确认 development silver 本身把衣物/食品当作场所风格依据，应先保留并标记
  旧参考的诊断限制，再对明确范围错误进行独立版本的自动参考修订。仍保留同一 60 张图
  和五维身份，不新增人工；v9 与候选必须按同一修订参考重新对照，字段支持变化单列，
  不用降低支持后的分数与旧分数横比来宣称提升。无可靠参考时不得锁定新候选。
- 新候选最终验收保留正式模型和当前 v9 两个配对基线：仍须满足原正式模型的严格提升
  条件，同时核对不低于 v9，不能用超过更早模型掩盖对当前候选的回退。只有两项检查及
  既有业务/交接验证都通过，才可称为新的可晋级自动 silver 候选。
- v11最终验收失败后继续自主优化：只使用原固定development检查可见场景功能与招牌
