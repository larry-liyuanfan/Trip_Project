...
- **以图搜商品：** 业态准确率 60.00%（support 110）；价位准确率 17.00%（support 100）；风格 macro F1 6.77%；设施 macro F1 3.67%。
- **智能售后：** 问题分类 71.33%；严重度 29.33%；关键信息 macro F1 24.05%；OCR recall 1.33%（support 75）。
- **多模态行程规划：** 约束识别 0.41%；行程要素完整度 20.00%；硬约束 macro F1 1.09%；软约束 macro F1 0%。
