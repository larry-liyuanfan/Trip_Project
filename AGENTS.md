...
The existing Week 3 human annotations are now frozen for reuse. Subsequent Week 3 execution must not require new annotation, relabeling, a second reviewer, adjudication, or manual completion of unknown or empty semantic fields before running. Preserve those limitations in counts, metrics, and the report; never replace them with generated labels. Existing traceability, immutable run records, Schema checks, evaluation exclusion checks, mock/dry/live modes, metric aggregation, and error export may be retained when they directly support the mentor deliverables, but they are not separate scope or additional release gates.

Run eligibility and delivery acceptance are distinct. Sampling strata may establish intended candidate coverage but must not be presented as human-gold coverage. Frozen samples may still be evaluated when structurally valid; if their human labels do not substantiate a mentor-required category or field, keep the affected metric unsupported and the delivery `PARTIAL` rather than requesting more annotation or claiming `READY`.
